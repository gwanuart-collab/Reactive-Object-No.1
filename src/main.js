  ...
  (gltf) => {
    ...
    if (circle1World) {
      ...
      if (rule) {
        ...
        pushMeshes.push(circle1World)
        pushBasePos.set(circle1World.uuid, circle1World.position.clone())
        pushMaxByUuid.set(circle1World.uuid, U(rule.max ?? BALLOON_PUSH_MAX))
        pushAxisByUuid.set(circle1World.uuid, rule.axis ?? BALLOON_PUSH_AXIS)
        pushSignByUuid.set(circle1World.uuid, rule.sign ?? 1)
      }
    }
